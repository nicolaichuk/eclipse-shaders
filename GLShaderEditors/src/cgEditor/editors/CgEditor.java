...
/**
 * Editor for the cg files
 * Use the cgScanner for syntax colorisation
 * @author Martinez
 *
 */
public class CgEditor extends TextEditor implements IPropertyChangeListener {
	...
	protected boolean affectsTextPresentation(PropertyChangeEvent event) {
		...
		{ affect = affect || event.getProperty().equals(PreferenceConstants.PREFERENCES[i][0])
			|| event.getProperty().equals(PreferenceConstants.PREFERENCES[i][1])
			|| event.getProperty().equals(PreferenceConstants.PREFERENCES[i][2]);
		}
