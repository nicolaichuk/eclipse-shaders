 ...
 * @see IWordDetector
 */
public class WordRuleEx implements IRule {
	 ...
	 */
	public WordRuleEx(IWordDetector detector) {
	 ...
	 * @since 3.3
	 */
	public WordRuleEx(IWordDetector detector, IToken defaultToken, boolean ignoreCase) {
	...
	public IToken evaluate(ICharacterScanner scanner) {
		...
		if (c != ICharacterScanner.EOF && fDetector.isWordStart((char) c)) {
			if (fColumn == UNDEFINED || (fColumn == scanner.getColumn() - 1)) {
				...
				if (token == null) {
					token= (IToken)fWords.get(buffer.replaceAll(" ", "").replaceAll("\t", ""));
				}
