...
public class CgEditorPlugin extends AbstractUIPlugin {
	...
	public void start(BundleContext context) throws Exception {
		...
		for(int i=0; i< PreferenceConstants.PREFERENCES.length; i++)
		{
			...
			RGB rgb = PreferenceConverter.getColor(getPreferenceStore(),PreferenceConstants.PREFERENCES[i][1]);
			int style = getPreferenceStore().getInt(PreferenceConstants.PREFERENCES[i][2]);
			token.setData(new TextAttribute(TokenManager.getColor(rgb), null, style));
		}
	}
