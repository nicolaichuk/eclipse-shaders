...
public class CgEditorPreferencePage extends PreferencePage implements
		...
		color.addListener(new IPropertyChangeListener()
				{
					...
					public void propertyChange(PropertyChangeEvent event) {
						...
						if(token!=null)
						{
							...
							TextAttribute att2 = new TextAttribute(TokenManager.getColor(rgb), null, att.getStyle());
							token.setData(att2);
						}
					}
	...
	protected void saveData() {
		...
		for (int i = 0; i < PreferenceConstants.PREFERENCES.length; i++) {
			Token token = TokenManager.getToken(PreferenceConstants.PREFERENCES[i][0]);
			TextAttribute att = (TextAttribute) token.getData();
			PreferenceConverter.setValue(store,PreferenceConstants.PREFERENCES[i][1], att.getForeground().getRGB());
			store.setValue(PreferenceConstants.PREFERENCES[i][2], att.getStyle());
		}
	}
